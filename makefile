...
USER_LD_FLAGS = 
USER_LIBS = 
MYOF = /home/matthew/work/eclipseOF/openframeworks/
...
CORE_INCLUDES = $(shell find $(MYOF)libs/openFrameworks/ -type d)
CORE_INCLUDE_FLAGS = $(addprefix -I,$(CORE_INCLUDES))
INCLUDES = $(shell find $(MYOF)libs/*/include -type d)

INCLUDES_FLAGS = $(addprefix -I,$(INCLUDES))
LIB_STATIC = $(shell ls $(MYOF)libs/*/lib/$(LIBSPATH)/*.a | grep -v openFrameworksCompiled | sed "s/.*\\/lib\([^/]*\)\.a/-l\1/")
LIB_SHARED = $(shell ls $(MYOF)libs/*/lib/$(LIBSPATH)/*.so | grep -v openFrameworksCompiled| sed "s/.*\\/lib\([^/]*\)\.so/-l\1/")

#LIB_PATHS_FLAGS = -L$(MYOF)libs/openFrameworksCompiled/lib/$(LIBSPATH)
LIB_PATHS_FLAGS = $(shell ls -d $(MYOF)libs/*/lib/$(LIBSPATH) | sed "s/\(\.*\)/-L\1/")
...
ifeq ($(findstring addons.make,$(wildcard *.make)),addons.make)
	ADDONS_INCLUDES = $(shell find $(MYOF)addons/*/src/ -type d)
	ADDONS_INCLUDES += $(shell find $(MYOF)addons/*/libs/ -type d)

	ADDONSCFLAGS = $(addprefix -I,$(ADDONS_INCLUDES))
	
	ADDONS_LIBS_STATICS = $(shell ls $(MYOF)addons/*/libs/*/lib/$(LIBSPATH)/*.a)
	ADDONS_LIBS_SHARED = $(shell ls $(MYOF)addons/*/libs/*/lib/$(LIBSPATH)/*.so)
	...
	ADDONS_DIRS = $(addprefix $(MYOF)addons/, $(ADDONS_REL_DIRS) )
	ADDONS_LIBS_DIRS = $(addprefix $(MYOF)addons/, $(ADDONS_LIBS_REL_DIRS) )
	...
	ADDONS_OBJFILES = $(subst $(MYOF), ,$(patsubst %.c,%.o,$(patsubst %.cpp,%.o,$(ADDONS_SOURCES))))
endif
...
after:
	cp -r $(MYOF)export/$(LIBSPATH)/libs bin/
	@echo
	@echo "     compiling done"
